...
void build(int node, int l, int r){
    ...
    tree[node] = min(tree[2*node], tree[2*node+1]);     //change op
}
...
void initialize(){
    ...
    iterador*=2;
    tree.resize(iterador-1, INFL);     //value of every node should be neutral value of op when itilializing
...
ll query(int node, int l, int r, int a, int b){     //call it like query(1, 0, arr.size()-1, a, b)
    ...
    int mid = (l+r)>>1;
    return min(query(2*node, l , mid, a, b), query(2*node+1, mid+1, r, a, b));  //change op
}
...
int main(){
    ...
    while(q--){
        ...
        if(t == 1){
            upd(1, 0, arr.size()-1, a, b);
        }else{
            cout << query(1,0, arr.size()-1, a, b-1) << endl;
        }
    }
