...
template <class T = int> struct Seg{
    int n, m = 1;
    T neutral=0;    //remember to change

    vector<int> tree, best, pre, suf, lazy;
    
    void join(const int x, int l, int r){
        int a = 2*x, b = 2*x+1;     
        
        tree[x] = tree[a] + tree[b];
        if(suf[a] && pre[b]) tree[x]--;

        best[x] = best[a] + best[b];
    ...
    void unlazy(int node, int l, int r){
        if(lazy[node] != -1){
            ...
            best[node] = tree[node] * (r-l+1);
            suf[node] = pre[node] = tree[node];
            if(l!=r){
...
signed solve(){
    ...
    while(q--){
        ...
        cout << seg.tree[1] << ' ' << seg.best[1] << endl;
    }
