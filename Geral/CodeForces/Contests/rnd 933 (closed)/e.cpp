...
template <class T> struct Seg{
    int n, m = 1;
    T neutral=1e17;    //remember to change
    vector<T> tree, lazy;
    ...
    void unlazy(int node, int l, int r){
        if(lazy[node] != neutral){
            tree[node]=min(lazy[node], tree[node]); //remember to change
            if(l!=r){
...
signed solve(){
    ...
    for(int j = 0; j < n; j++){
        ...
        for(int i = 0; i <= d+1 && i < m; i++) seg.upd(i+1, 0);

        for(int i = 1; i < m-1; i++){
            ll cc = seg.query(i+1, i+1);
