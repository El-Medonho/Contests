...
ll sums(int ind, vector<ll> &bit){         //O(logn)
    ll ans=0;
    while(ind){
        ans+=bit[ind];    
        ind-=ind&-ind;    
...
signed solve(){
    ...
    for(int i = 0; i < n; i++){
        ...
        upd(mp[arr[i].first], 1, bit);
        upd(mp[arr[i].second+1], -1, bit);
        mt.insert(arr[i].first);
        mt.insert(arr[i].second);
    }

    for(ll i: mt) {ans = max(ans, sums(mp[i], bit)); ans = max(ans, sums(mp[i+1], bit));}
