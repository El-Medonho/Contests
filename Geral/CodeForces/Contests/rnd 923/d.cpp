...
template <class T = pair<ll,ll>> struct Seg{
    int n, m = 1;
    T neutral= make_pair(mx, mx);    //remember to change
    vector<T> tree, lazy;
    T join(const T &a, const T &b){   
        if(a.first == 1e12) return b;
        if(b.first == 1e12) return a;  
        return (arr[a.first] == arr[b.first] ? make_pair(a.first, min(a.second, b.second)) : make_pair(a.first, b.first));      //remember to change
    }
    Seg(int n, vector<int> ini = {}) : n(n) {
        while(m < n) m<<=1;
        ...
        if(ini.size()){
            for(int i = m; i < m+ini.size(); i++) tree[i].first = i-m;
