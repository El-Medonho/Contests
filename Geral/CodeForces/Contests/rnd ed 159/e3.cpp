...
int n;
vector<int> acc_hash, acc_inv_hash;
hashes_t2(const string &_s): s(_s), n(s.size()), acc_hash(n
+ 1, 0)
, acc_inv_hash(n + 1, 0) { 
for (int i = 0; i < n; ++i) {
acc_hash[i + 1] =
(acc_hash[i] + (ll)base_pow2[i] * (s[i] - 'a'
+ 1)) % mod2;
acc_inv_hash[i + 1] = (acc_inv_hash[i] + (ll)inv_base_pow2[i] * (s[i
] - 'a' + 1)) % mod2;
}
}
int get_hash(int a, int b) { // 04a73b
...
signed solve(){
    ...
    ll tot = 0;
    for(int i = 0; i < n; i++) {tot += s[i].size(); hs[i] = hashes_t2(s[i]); hsn[i] = hashes_t(s[i]);}
