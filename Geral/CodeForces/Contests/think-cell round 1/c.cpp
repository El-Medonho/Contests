...
template <class T = pair<ll,ll>> struct Seg{
    int n, m = 1;
    T neutral={-1e18, -1e18};    //remember to change
    vector<T> tree;
    vector<ll> lazy;
    ...
    Seg(int n, vector<ll> ini = {}) : n(n) {
        ...
        if(ini.size()){
            for(int i = m; i < m+ini.size(); i++) tree[i] = {ini[i-m] + (i-m+1), i-m+1};
    ...
    void upd(int node, int l, int r, int pos, const ll &val){
        if(l==r){
            tree[node].first+=val;     //change if necessary
            return;
        }
        int mid=(l+r)>>1;
        if(pos<=mid){
    ...
    void upd(int node, int l, int r, int a, int b, const ll &val){
        unlazy(node, l, r);
        if(a>r || b<l) return;
        if(l>=a && r<=b){
            lazy[node]=val;
    ...
    void upd(int l, int r, const ll &val){
        upd(1, 1, m, l, r, val);
    }
    T query(int l, int r){
...
signed solve(){
    ...
    for(int i = 0; i < n; i++){
        pair<ll,ll> h = seg.query(1, n);

        st.insert(h.first);

        seg.upd(h.second, -1e13);
        if(h.second != n) seg.upd(h.second, n, -1);
    }
