...
signed solve(){
    ...
    ll ans = 1;
    vector<int> currvis(n);
    vector<vector<int>> caracor(n+2);

    vector<ll> bit(2*n+5, 0), in(n), out(n);
    int cnt = 1;
    ...
    dfs2 = [&](int cc){
        int h = caracor[cor[cc]].size() ? currvis[caracor[cor[cc]].back()] : 0;
        ...
        caracor[cor[cc]].push_back(cc);

        for(int j: graph[cc]){

        }

        caracor[cor[cc]].pop_back();
    }
