...
signed solve(){
    ...
    vector<vector<int>> nums(1e6+5);

    for(int i = 1; i < 1e6+2; i++){
        for(int j = 0; j < 27; j++){
            nums[i].push_back(uni(rng));
        }
    }

    vector<vector<int>> arr(n, vector<int>(27, 0));
    vector<int> sums(27, 0);
    for(int i = 0; i < n; i++){
        int x; cin >> x;

        vector<ull> res = factor(x);
        for(int j = 0; j < 27; j++){
            int sum = 0;
            for(int k: res){
                sum = (sum + nums[k][j]) % 3;
            }
            arr[i][j] = (sum + sums[j]) % 3;
            sums[j] = arr[i][j];
        }
    }
    ...
    while(q--){
        ...
        bool ok = true;
        for(int j = 0; j < 27; j++){
            int h = ( (arr[r][j] - (l == 0 ? 0 : arr[l-1][j]) ) % 3 + 3) % 3;
            if(h != 0){
                ok = false;
                break;
            }
